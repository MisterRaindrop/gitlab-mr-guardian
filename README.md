...
**开关持久，进程不常驻。**`start` / `stop` 的开关状态写在插件数据目录的 `control.json` 里，关闭 Claude Code、重启机器都不会丢，直到显式执行 `/gitlab-mr-guardian:stop`。但轮询进程本身只在有 Claude Code 交互式会话打开时运行：开关开着的情况下，任何新开的会话都会自动开始轮询，不需要每次重新 `start`；单纯关掉窗口只是暂停，不等于关闭监控。

**多会话时由文件锁决定谁干活。**每个开着的会话都会启动自己的 Monitor，但它们竞争同一把文件锁——只有持有锁的会话真正执行轮询和操作（rebase、触发 CI、auto-merge），其他会话待命，避免对同一 GitLab 主机重复操作。

**关掉一个会话，其他会话会接管。**持有锁的会话关闭后锁随之释放，其他仍开着的会话中会有一个在它的下一个轮询周期抢到锁并继续监控。注意两点：

- **接管有延迟**：待命会话按轮询间隔（默认 1 小时）醒来试锁，所以最坏情况下要等约一个轮询周期才完成接管，中间存在空窗，不是秒级无缝切换。
- **通知跟着锁走**：轮询结果和 CI 告警只出现在持有锁的那个会话里，接管发生后告警会改在新会话中出现。
