...
| `rebase_when_ci_failed` | `false` | Allow the safe rebase flow when GitLab reports `need_rebase` while the current pipeline has not yet succeeded (failed, running, missing, or skipped). A running pipeline is built on the stale base and its result is outdated either way, so the rebase takes priority and triggers a fresh pipeline on the new base. Requires `auto_rebase`; all approval safety checks still apply. |
